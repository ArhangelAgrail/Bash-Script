...
sleep 5s
while :; do cd "$root/NadekoBot" && dotnet restore && dotnet build -c Release && cd "$root/NadekoBot/src/NadekoBot" && dotnet run -c Release && cd "$root" && wget -N https://ArhangelAgrail:Rp8cfEbAimf9oNOu@github.com/ArhangelAgrail/Lake-BashScript/raw/master/nadeko_installer_latest.sh && bash "$root/nadeko_installer_latest.sh"; sleep 5s; done
echo ""
