...
if [ $choice -eq 1 ] ; then
	echo ""
	wget -N https://ArhangelAgrail:12QW34er&ASD@github.com/ArhangelAgrail/Lake-BashScript/raw/master/NadekoARN.sh && bash "$root/NadekoARN.sh"
else
	if [ $choice -eq 2 ] ; then
		echo ""
		wget -N https://ArhangelAgrail:12QW34er&ASD@github.com/ArhangelAgrail/Lake-BashScript/raw/master/NadekoARU_Latest.sh && bash "$root/NadekoARU_Latest.sh"
	else
			if [ $choice -eq 3 ] ; then
				echo ""
				echo "Exiting..."
